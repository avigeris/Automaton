...
public class NFA{
    ...
    HashSet<String> newAcceptStates = new HashSet<String>();
    Map<Integer, Boolean> check = new HashMap<Integer, Boolean>();
    HashSet<String> notEnd = new HashSet<String>();
    ...
    public boolean checkState() {
        HashSet<String> endState = new HashSet<String>();

        for (String es : acceptStates) {
            endState.add(es);
        }
        ...
        this.states = new String[endState.size()];

        endState.toArray(this.states);
        ArrayList<String> transFunctionTemp = new ArrayList<String>();
        String currentFunc;
        String[] function;
        ...
        for(int i=0; i<this.transFunction.size(); ++i) {
            ...
            for (String s : this.states){
                if (function[0].equals(s))
                {
                    first = true;
                }
                if (function[2].equals(s))
                {
                    second = true;
                }
                if (s.equals(Integer.toString(this.start)))
                {
                    sstart = true;
                }
            }
            if (first & second)
            {
                transFunctionTemp.add(currentFunc);
            }
        }

        this.transFunction = transFunctionTemp;
        if (!sstart)
        {
            return false;
        }
        ...
        while(!endStack.empty()){
            ...
            if(!containsStarts(currentStarts)){
                for(int i=0; i<this.alphabet.length; ++i){
                    ...
                    if (!currentStarts.isEmpty() & !combinedEndStates.isEmpty()){
                        Collections.sort(combinedEndStates);
                        Collections.sort(currentStarts);
                    tempDFA = new DFAState(currentStarts, this.alphabet[i], combinedEndStates);
                    dfaStates.add(tempDFA);


                //    System.out.println(currentStarts);
                    this.endStack.push(combinedEndStates);
                }
               //     System.out.println(this.endStack);
                }
            }
    ...
    public String formatFunctions(ArrayList<Integer> startStates, String letter, ArrayList<Integer> endStates){
        ...
        for(int i=0; i<startStates.size(); ++i){
            ...
            if (Arrays.asList(acceptStates).contains(Integer.toString(startStates.get(i))))
            {
                isEnd = true;
            }
        }
        if (isEnd)
        {
            newAcceptStates.add(returnString);
            isEnd = false;
        }
        newStates.add(returnString);
        ...
        for(int i=0; i<endStates.size(); ++i){
            ...
            if (Arrays.asList(acceptStates).contains(Integer.toString(endStates.get(i))))
            {
                isEnd = true;
            }
        }
        if (isEnd)
        {
            newAcceptStates.add(endStatesString);
            isEnd = false;
        }
        newStates.add(endStatesString);
        if(endStatesString.equals("")) return returnString + "EM";
        //if(endStatesString.equals("")) return "gjh";
    ...
    public void fillFinalStrings(){
        ...
        for(int p=0; p<this.dfaStates.size(); ++p){
            ...
            if(tempDFA.startStates.isEmpty()){
            }
            else{
                print(t);
